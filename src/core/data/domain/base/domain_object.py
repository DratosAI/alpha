...
class DomainObjectFactory(Generic[T]):
    @staticmethod
    def create_from_data(data: Dict[str, Any], object_type: str) -> T:
        class_type = DomainObjectFactory.get_class_by_type(object_type)
    ...
    def get_class_by_type(object_type: str) -> Type[T]:
        for name, obj in inspect.getmembers(importlib.import_module(__name__)):
            if (
                inspect.isclass(obj)
                and issubclass(obj, DomainObject)
                and hasattr(obj.Meta, "type")
            ):
